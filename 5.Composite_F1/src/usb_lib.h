...
#define ENDP_DTOG_RX(num, dtog) do{USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_TX | USB_EPRX_STAT | USB_EPTX_STAT)) | USB_EP_CTR_RX | USB_EP_CTR_TX) ^ (dtog*USB_EP_DTOG_RX); }while(0)
#define ENDP_DTOG_TX(num, dtog) do{USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_RX | USB_EPRX_STAT | USB_EPTX_STAT)) | USB_EP_CTR_RX | USB_EP_CTR_TX) ^ (dtog*USB_EP_DTOG_TX); }while(0)
...
static inline void usb_ep_write(uint8_t epnum, const uint16_t *buf, uint16_t size){
  ...
  ENDP_STAT_TX((epnum & 0x0F), USB_EP_TX_VALID);
}
extern uint8_t flag;
